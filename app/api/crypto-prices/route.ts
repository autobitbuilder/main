...
// Simulated cryptocurrency data for demo purposes
const simulatedCryptoData = [
  {
    id: "bitcoin",
    name: "Bitcoin",
    symbol: "BTC",
...
export async function GET() {
  try {
    // Simulate some price variation for demo purposes
    const simulatedData = simulatedCryptoData.map((crypto) => ({
      ...crypto,
      price: crypto.price * (0.98 + Math.random() * 0.04), // ±2% variation
      change24h: crypto.change24h + (Math.random() - 0.5) * 1, // ±0.5% variation
    }))
